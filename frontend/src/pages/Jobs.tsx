...
export default function Jobs() {
  ...
  const deleteJob = async (id: string, title: string) => {
    try {
      ...
      toast({ title: "Vaga removida", description: `"${title}" foi removida.` });
    } catch (e: any) {
      toast({ title: "Erro", description: e.message, variant: "destructive" });
    }
  };
      ...
      {jobs.length === 0 ? (
        ...
        <div className="grid gap-4">
          {jobs.map((job) => {
            ...
            return (
              <div key={job.id} className="bg-card rounded-2xl border border-border p-6 hover:shadow-elevated transition-all duration-300">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />{job.resumes_count ?? 0} candidatos</span>
                    </div>
                    ...
                    {(job.resumes_count ?? 0) > 0 && (
                      <span className="px-2.5 py-1 rounded-full bg-success/10 text-success text-xs font-medium">{job.resumes_count} currículos</span>
                    )}
                  </div>
                </div>
